...
def get_paths():
    ...
    root.geometry("%sx%s+%s+%s" % (450, 300, x, y))
    ...
    Label(root, text="Please assign the directory to put the processed data: ").grid(row=4, padx=10, pady=5, sticky=W)
    Entry(root, textvariable=tk_target_dir).grid(row=5, column=0, padx=10, sticky=W+E)
    Button(root, text="Browse Computer", command=lambda: select_dir(tk_target_dir)).grid(row=5, column=1, padx=10, pady=10)
    Button(root, text="Confirm and Start", command=root.quit).grid(row=7, columnspan=5, padx=10, pady=10, sticky=W+E)
    root.mainloop()
    ...
    return file_loc_str, temp_loc_str, target_dir_str
